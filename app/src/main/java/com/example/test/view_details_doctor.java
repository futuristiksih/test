...
public class view_details_doctor extends Fragment {
    ...
    CheckBox breast_feedC,vomitC,dehydrationC;
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        immunization.setEnabled(false);bowel_movement.setEnabled(false);fever.setEnabled(false);
        inception.setEnabled(false);infected_area.setEnabled(false);
        intake.setEnabled(false);environment.setEnabled(false);crying.setEnabled(false);
        breast_feedC.setEnabled(false);vomitC.setEnabled(false);dehydrationC.setEnabled(false);
        dob.setEnabled(false);birth_weight.setEnabled(false);name.setEnabled(false);
        ...
        db.collection("Email").document("parent "+parent_email).collection("sent_appointments")
                .document(child_name+" "+email).collection("Dates").document(id).get()
                .addOnSuccessListener(documentSnapshot -> {
                    db.collection("Email").document("parent "+parent_email).collection("sent_appointments")
                            .document(child_name+" "+email).get().addOnSuccessListener(documentSnapshot1 -> {
                                dob.setText(documentSnapshot1.get("dob").toString());name.setText(child_name);
                   ...
                   childDetails details=new childDetails(child_details.get("immunization").toString(), child_details.get("bowel_movement").toString(),
                           child_details.get("fever").toString(),child_details.get("inception").toString(),
                           child_details.get("infected_area").toString(),child_details.get("intake").toString(),child_details.get("environment").toString(),
                           child_details.get("crying").toString(),child_details.get("vomit").toString(),child_details.get("breast_feed").toString(),
                           child_details.get("dehydration").toString());
        ...
        db.collection("Email").document("parent "+parent_email).get().addOnSuccessListener(documentSnapshot ->
        ...
        chat.setOnClickListener(view12 -> {
            ...
            startActivity(chatIntent);getActivity().finish();
        });
        diagnosis.setOnClickListener(view1 -> {
            Bundle bundle1=new Bundle();
            bundle1.putString("child_name",child_name);bundle1.putString("parent_email",parent_email);bundle1.putString("id",id);bundle1.putString("user","d");
